...
func TranscodeToStandard(videoName string, filename string, folderPath string) {
	...
	ffmpegCommand480Pass2 := fmt.Sprintf("ffmpeg -i %s/%s -codec:v libx264 -profile:v high -preset slow -b:v 500k -maxrate 500k -bufsize 1000k -vf scale=-2:480 -threads 0 -pass 2 -codec:a libfdk_aac -b:a 128k -f mp4 %s/%s_480p.mp4", folderPath, filename, folderPath, videoName)
...
func TranscodeToMobile(videoName string, filename string, folderPath string) {
	...
	ffmpegCommand360Pass2 := fmt.Sprintf("ffmpeg -i %s/%s -codec:v libx264 -profile:v baseline -level 3.1 -preset slow -b:v 250k -maxrate 250k -bufsize 500k -vf scale=-2:360 -threads 0 -pass 2 -codec:a libfdk_aac -b:a 128k -f mp4 %s/%s_360p.mp4", folderPath, filename, folderPath, videoName)
...
func TranscodeToHighSD(videoName string, filename string, folderPath string) {
	...
	ffmpegCommand576Pass2 := fmt.Sprintf("ffmpeg -i %s/%s -codec:v libx264 -profile:v high -preset slow -b:v 1000k -vf scale=-2:576 -threads 0 -pass 2 -codec:a libfdk_aac -b:a 196k -f mp4 %s/%s_576p.mp4", folderPath, filename, folderPath, videoName)
